...
function handleClick(event) {
  ...
  if (isPointInsideObject( game.player, clickX, clickY, )) {
    showModal(`hah-yit`, `<span style="color: saddleBrown">  Carga de Lanzallamas -> ${game.player.fireAmount}</span>  <span style="color: darkblue">  Carga eléctrica -> ${game.player.electroAmount}</span>  <span style="color: darkred">   Cantidad de blasters -> ${game.player.blasterAmount}</span>  <span style="color: darkgreen">   Cantidad de ganchos -> ${game.player.hookAmount}</span>     <span style="color: darkgreen">   Resistencia de las barras -> ${barLife}</span>     
    <br/> <span style="color: green">  Nivel del arma básica -> ${basicWeaponLevel} </span> <span style="color: green">  Duración de escudos y del daño eléctico-> ${shieldsDuration/1000}s </span>   
    <span style="color: green">  Duración de escudos y del daño eléctico-> ${shieldsDuration/1000}s </span>  
