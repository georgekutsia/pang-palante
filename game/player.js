class Player {
  ...
  draw() {
    this.bulletArray.forEach((bullet) => {
      bullet.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.bulletFireArray.forEach((bullet) => {
      bullet.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.bulletBarArray.forEach((bullet) => {
      bullet.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.bulletPlatformArray.forEach((bullet) => {
      bullet.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.hooksArray.forEach((bullet) => {
      bullet.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.swordArray.forEach((bullet) => {
      bullet.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.life.draw();
    ...
    if (this.swordEquipped === true) {
      ...
      if (this.swordLevel >= 0)
        this.swordInfo$$.innerText = ` lvl ${this.swordLevel}`;
    ...
    if (this.weaponUpgrading) {
      ...
      this.ctx.drawImage(
        this.basicWeapon,
        CTXW - 100 - weaponImgSizing,
        CTXH - 100 - weaponImgSizing,
        100 + weaponImgSizing,
        100 + weaponImgSizing
      );
      this.ctx.globalAlpha = 1;
    }
    if (this.electroAmount <= 0) {
      this.electricShieldIsActive = false;
      H = 0;
      this.electricBurbalasIsActive = false;
    }
    ...
    if (recharge <= 200) {
      this.ctx.drawImage(
        this.machinegunPaint,
        this.x,
        this.y + 5,
        this.ctx.canvas.width / 30,
        this.ctx.canvas.width / 26
      );
    }
    ...
    this.itemTakens.forEach((sparkle) => {
      sparkle.draw();
    }); // paso 3: dibujo cada bullet que se dispare
    this.swordSparkles.forEach((swo) => {
      swo.draw();
    }); // paso 3: dibujo cada bullet que se dispare
  }
  ...
  move() {
    ...
    if (
      (this.vx && this.x + this.h <= CTXH - 5) ||
      (this.vy && this.x + this.h <= CTXH - 5)
    ) {
    ...
    this.bulletArray.forEach((bullet) => {
      bullet.move();
    }); //paso 4: mueve cada bullet que se dispare
    this.bulletFireArray.forEach((bullet) => {
      bullet.move();
    }); //paso 4: mueve cada bullet que se dispare
    this.bulletBarArray.forEach((bullet) => {
    ...
    this.swordArray.forEach((sparkle) => {
      sparkle.move();
    }); // paso 3: dibujo cada bullet que se dispare
    this.itemTakens.forEach((sparkle) => {
      sparkle.move();
    }); // paso 3: dibujo cada bullet que se dispare
    this.swordSparkles.forEach((sparkle) => {
      sparkle.move();
    }); // paso 3: dibujo cada bullet que se dispare
    ...
    if (this.swordPowerUp >= powerToGetForSword) {
      ...
      if (this.swordSparkleActive) {
        ...
        for (let i = 0; i < 9; i++) {
          let swordSparkle2 = new BurningColors(
            this.ctx,
            CTXW - 43,
            -40 + i * 9,
            110,
  ...
  loseLife(damage, immuneState) {
    ...
    setTimeout(() => {
      // para desactivar immune y que el personaje deje de parpadear
  ...
  shoot() {
    // paso 1: invoca el disparo desde la posicion del personaje o su cercanía
    if (this.shootUp) {
      if (finalBoss) {
        this.img.src = "../public/Imagenes/pangPjNuevoDerecha3.png";
      }
      const bullet = new BasicWeapon(
        this.ctx,
        this.x + this.w / 1.8,
        this.y,
        bulletDirection
  ...
  shootTriple() {
    ...
    if (this.shootUp) {
      const bullet1 = new BasicWeapon(
        ...
        this.y,
        bulletDirection,
        -1 - basicWeaponSpeed / 2
      );
      const bullet2 = new BasicWeapon(
        this.ctx,
        this.x + this.w,
        this.y,
        bulletDirection,
  ...
  shootCuatruple() {
    ...
    if (this.bigWeaponBubblesMaxAmount <= 7) {
      ...
      if (this.bigWeaponBubblesMaxAmount < 0)
        this.bigWeaponBubblesMaxAmount = 0;
  ...
  swingSword() {
    ...
    if (this.swingSwordState) {
      if (!this.electricShieldIsActive) {
        let sword = new WeaponSword(
          this.ctx,
          this.x - 60,
          this.y - 70,
          this.swingSwordState,
          0,
          0,
          0,
          false
  ...
  shootStep(stepPlace, where) {
    if (this.platformCreator && this.stepsAmount > 0) {
      ...
      const plat = new Platform(
        ...
        65,
        10,
        "/public/Imagenes/obstacles/stepsSolid2.png",
        true,
        true,
        true,
        0,
  ...
  // acciones para teclado y táctil
  // acciones para teclado y táctil
  moveW() {
    this.keySwitchCounter = 0;
    if (this.canClimb) {
